const UserCtrl = (function() {
  ...
  const userApiCall = (input) => {
    axios.get('http://this-is-hangman.herokuapp.com/user?' + 'nickname=' + input.nickname + '&secret=' + input.secret)
    .then( (resp) => {
      console.log(resp)
  ...
  const updateUserScore = () => {
    ...
    axios.put('http://this-is-hangman.herokuapp.com/user?' + 'id=' + userObj.id + '&score=' + userObj.score)
    .then( (resp) => {
      console.log(resp);
    })
    .catch( (err) => {
